resource "aws_security_group" "elb-public-admin" {
	name = "${var.environment}.jra-sg.${var.region}-admin-elb"
	...
	tags {
        Name = "${var.environment}.sg.${var.region}-admin-elb",
...
# ---------------------------------------------------------------------------
resource "aws_elb" "elb-public-admin" {
    name = "${var.environment}-jra-elb-${var.region}-admin"
    subnets = ["${aws_subnet.az-1-public.id}"]
    security_groups = ["${aws_security_group.elb-public-admin.id}"]
