resource "aws_security_group" "elb-public-app-api" {
	name = "${var.environment}.jra-sg.${var.region}-app-api-elb"
	...
	tags {
        Name = "${var.environment}.sg.${var.region}-app-api-elb",
...
# ---------------------------------------------------------------------------
resource "aws_elb" "elb-public-app-api" {
    name = "${var.environment}-jra-elb-${var.region}-app-api"
    subnets = ["${aws_subnet.az-1-public.id}"]
    security_groups = ["${aws_security_group.elb-public-app-api.id}"]
