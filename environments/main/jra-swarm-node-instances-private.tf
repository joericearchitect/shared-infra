...
resource "aws_instance" "infra-load-az-1" {
	ami = "${lookup(var.amis_docker_node, "${var.region}")}"
	availability_zone = "${lookup(var.availability_zone, "${var.region}.az-1")}"
	instance_type = "${lookup(var.ec2-instance-type, "${var.environment-size}.infra-load")}"
	key_name = "${var.aws_key_name}"
	security_groups = ["${aws_security_group.private_infra-load.id}"]
	subnet_id = "${aws_subnet.az-1-private.id}",
	monitoring = "true",
	provisioner "local-exec" {
       command = "echo "
	...
	tags {
        Name = "${var.environment}.instance.swarm-worker.infra-load.${var.environment}.${lookup(var.availability_zone, "${var.region}.az-1")}",
        jra.instance-name-full = "${var.environment}.instance.swarm-worker.infra-load.${var.environment}.${lookup(var.availability_zone, "${var.region}.az-1")}"
        jra.instance-name = "infra-load-${var.region}-az-1"
        jra.environment = "${var.environment}"
        jra.environment_type = "${var.environment_type}"
        jra.environment-instance-id = "${random_id.env-instance.b64}"
    	jra.failure-zone = "${var.region}-az-1"
    	jra.environment-size = "${var.environment-size}"
    	jra.swarm-instance-type = "swarm-worker"
    	jra.swarm-node-type = "infra-load"
    	jra.subnet-type = "private"
    }
}
...
# ---------------------------------------------------------------------------
# Swarm Node Instances for Monitoring - Promethus, Grafana, etc.
#  - EC2 Instances for across 3 avail zones
# ---------------------------------------------------------------------------
resource "aws_instance" "infra-monitoring-az-1" {
	ami = "${lookup(var.amis_docker_node, "${var.region}")}"
	availability_zone = "${lookup(var.availability_zone, "${var.region}.az-1")}"
	instance_type = "${lookup(var.ec2-instance-type, "${var.environment-size}.infra-monitoring")}"
	key_name = "${var.aws_key_name}"
	security_groups = ["${aws_security_group.private_infra-monitoring.id}"]
	subnet_id = "${aws_subnet.az-1-private.id}",
	monitoring = "true",
	provisioner "local-exec" {
       command = "echo "
	...
	tags {
        Name = "${var.environment}.instance.swarm-worker.infra-monitoring.${var.environment}.${lookup(var.availability_zone, "${var.region}.az-1")}",
        jra.instance-name-full = "${var.environment}.instance.swarm-worker.infra-monitoring.${var.environment}.${lookup(var.availability_zone, "${var.region}.az-1")}"
        jra.instance-name = "infra-monitoring-${var.region}-az-1"
        jra.environment = "${var.environment}"
        jra.environment_type = "${var.environment_type}"
        jra.environment-instance-id = "${random_id.env-instance.b64}"
    	jra.failure-zone = "${var.region}-az-1"
    	jra.environment-size = "${var.environment-size}"
    	jra.swarm-instance-type = "swarm-worker"
    	jra.swarm-node-type = "infra-monitoring"
    	jra.subnet-type = "private"
    }
}
