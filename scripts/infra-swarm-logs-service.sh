...
if [ -z "$1" ]
then
	$DIR/jra-util/jra-util-print-service-usage-info.sh $SCRIPT_NAME "<GREP_STRING> <-f>"
	exit 1;	
fi
...
then
	FOLLOW_STRING=" -f "
	FOLLOW_STRING_DISPLAY="grep $2";
else
	FOLLOW_STRING=''
	FOLLOW_STRING_DISPLAY="<no -f flag>";
fi
...
echo +   - Using this follow flag:   $FOLLOW_STRING_DISPLAY
echo +
echo +  Command executed:  
echo +   - ssh-aws.sh $SWARM_MANAGER_NODE "docker service logs $FOLLOW_STRING $SERVICE_NAME $GREP_STRING"
echo +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
printf '%b\n' ""
ssh-aws.sh $SWARM_MANAGER_NODE "docker service logs $FOLLOW_STRING $SERVICE_NAME $GREP_STRING"
printf '%b\n' ""