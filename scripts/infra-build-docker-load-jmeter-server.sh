...
echo .
echo -------------------------------------------------------------
echo   Building Jarch Load JMeter Server Docker Image...
echo ------------------------------------------------

REPO_NAME=$JRA_GLOBAL_DOCKER_REPO_NAME
IMAGE_NAME=$JRA_GLOBAL_IMAGE_NAME_PREFIX_INFRA-jmeter-server
IMAGE_VERSION=1.0.0
IMAGE_TAG_VERSION=$IMAGE_VERSION
IMAGE_TAG_LAGEST=latest

cd $INFRA_MODULES_LOAD_TESTING_DOCKER_DIR/jmeter-server
...
echo .
echo -------------------------------------------------------------
echo   Pushing Jarch JMeter Server Docker Image to Registry...
echo ------------------------------------------------
